...
class Cache(object):
    ...
    def update(self, key, element):
        ...
        """
        debug_str = "updating cahce element: {element} with key: {key}."

        logger.debug(debug_str.format(key=key.hashkey, elment=element))
    ...
    def get(self, key):
        ...
        debug_str = "attaining element: {element} with key: {key} \
                     resulted in a KeyError. Please check the key."
        try:
            response = self.cache[key.hashkey]
        except KeyError:
            logger.debug(debug_str.format(element=element, key=key.hashkey), exc_info=1)
            response = None
